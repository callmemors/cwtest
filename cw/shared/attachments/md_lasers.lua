...
if CLIENT then
	...
	att.description = {{t = "Hold the same category button to change color.", c = CustomizableWeaponry.textColors.INFO}}
	...
	function att:elementRender()
		...
		if util.PointContents(tr.HitPos) != CONTENTS_SOLID and not self.NearWall then
			...
			render.DrawBeam(laserPos + fw, laserPos + fw * dist, 0.08, 0, 0.99, renderColor)
			...
			if GetConVarNumber("cw_laser_blur") >= 1 then
				render.DrawBeam(self.lastLaserPos, tr.HitPos, 0.7, 0, 0.99, renderColor)
				...
				if dist < 2 then
					...
					render.DrawSprite(tr.HitPos, 0.3, 0.3, renderColor)
					renderColor.a = 80 * dist
					render.DrawSprite(tr.HitPos, 1.1, 1.1, renderColor)
					
					if laserHQ then
						renderColor.a = 25 * dist
						render.DrawSprite(tr.HitPos, 2, 2, renderColor)
					end
				end
			else
...
att.displayName = "Insight x2 Sub-Compact"
att.displayNameShort = "Ix2"
att.laserRange = 4096
att.laserBeamRange = 75
att.colorType = CustomizableWeaponry.colorableParts.COLOR_TYPE_BEAM
...
if CLIENT then
	...
	function att:elementRender()
		...
		if util.PointContents(tr.HitPos) != CONTENTS_SOLID and not self.NearWall then
			...
			render.DrawBeam(laserPos + fw, laserPos + fw * dist, 0.08, 0, 0.2, renderColor)
			...
			if laserHQ then
				...
				renderColor.a = 10
				render.DrawBeam(laserPos + fw, laserPos + fw * dist, 2, 0, 0.99, renderColor)
			end
