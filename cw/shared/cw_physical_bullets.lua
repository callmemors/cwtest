...
-- if GetConVarNumber("cw_physical_bullets") > 0 then
...
CustomizableWeaponry.MUZZLE_VELOCITY_MULTIPLIER = 5
...
function CustomizableWeaponry:processPhysicalBullet(bullet, dt)
	...
	bullet.velocity = math.Approach(bullet.velocity, bullet.initialVelocity * 0.3, dt * 10000) -- lose velocity over the flight
			...
			if not IsValid(ply) then -- player does not exist, don't draw his bullets
				...
				for key, bulletData in ipairs(data) do
					...
					if bulletData.isTracer then
						...
						render.DrawSprite(pos + norm * 128, 20, 20, clr)
						render.DrawBeam(pos + norm * 256, pos, 10, 0, 1, clr)
						else
						
						render.SetMaterial(bulletTracer)
						render.DrawBeam(pos + norm * 500, pos, 2, 0, 1, clr)
					end
				end
			end
		end
