...
function SWEP:DrawHUD()
	...
	if not self.dt.BipodDeployed then 
		if self.BipodInstalled then
			if self:CanRestWeapon(self.BipodDeployHeightRequirement) then
				...
				surface.SetDrawColor(0, 0, 0, 255)
				surface.DrawTexturedRect(x / 2 + 649, y / 2 + 179, 96, 96)
				
				surface.SetDrawColor(255, 255, 255, 255)
				surface.DrawTexturedRect(x / 2 + 650, y / 2 + 180, 96, 96)
			end
		else
			if self.dt.State == CW_AIMING then
	...
	-- if self.AimBreathingEnabled then
		self.HUD_BreathAlpha = LerpCW20(FT * 12, self.HUD_BreathAlpha, self.BreathFOVModifier)
		
			surface.SetDrawColor(0, 0, 0, 255 * self.HUD_BreathAlpha * 0.115)
		...
		if self.dt.State == CW_AIMING then
			if self.Owner:GetVelocity():Length() < self.BreathHoldVelocityMinimum then
				...
				-- draw.ShadowText(self:getKeyBind("+speed") .. " - steadyy aim", cwhud24, x / 2, y / 2 + 120, finalColorMain, Black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
