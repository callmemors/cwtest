...
function SWEP:applyOffsetToVM()
	...
	if CT > self.AimTime then
		if ((self.MoveWepAwayWhenAiming and self.dt.State == CW_AIMING) or (self:canUseSimpleTelescopics() and self.dt.State == CW_AIMING)) and (self.dt.M203Active and (not self.M203Chamber)  or not self.dt.M203Active) then
			self.ViewModelFOV = 90
			pos = pos * ang:Forward() * 100
			-- return pos, ang
		end
	...
	if not self.ViewModelFlip then
			if self:isAiming() then
				RotateAroundAxis(ang, Up(ang), self.BlendAng.y + self.RecoilAng.y - self.AngleDelta.y * -0.3 * swayIntensity)
				RotateAroundAxis(ang, Forward(ang), self.BlendAng.z + self.RecoilAng.r + self.AngleDelta.y * 0.5 * swayIntensity)
			else
				RotateAroundAxis(ang, Up(ang), self.BlendAng.y + self.RecoilAng.y - self.AngleDelta.y * -0.3 * swayIntensity)
				RotateAroundAxis(ang, Forward(ang), self.BlendAng.z + self.RecoilAng.r + self.AngleDelta.y * 0.2 * swayIntensity)
			end
	else
			if self:isAiming() then
				RotateAroundAxis(ang, Up(ang), -self.BlendAng.y + self.RecoilAng.y - self.AngleDelta.y * -0.3 * swayIntensity)
	...
	if not self.ViewModelFlip then
			if self:isAiming() then
		pos = pos + (self.BlendPos.x + self.AngleDelta.y * 0.01 * swayIntensity + self.RecoilPos.z) * Right(ang)
			else
		pos = pos + (self.BlendPos.x + self.AngleDelta.y * -0.1 * swayIntensity + self.RecoilPos.z) * Right(ang)
			end
	else
			if self:isAiming() then
		pos = pos - (self.BlendPos.x - self.AngleDelta.y * 0.01 * swayIntensity - self.RecoilPos.z) * Right(ang)
	...
	pos = pos + (self.BlendPos.y - self.FireMove - self.RecoilPos.y) * Forward(ang)
			if self:isAiming() then
				...
				-- RotateAroundAxis(ang, Up(ang), self.BlendAng.z * -0.1)
			else
				pos = pos + (self.BlendPos.z - self.AngleDelta.p * 0.1 * swayIntensity - self.RecoilPos.z) * Up(ang)
				-- RotateAroundAxis(ang, Up(ang), self.BlendAng.z * -0.1)
			end
...
function SWEP:draw3D2DHUD()
	...
	cam.Start3D2D(att.Pos + ang:Forward() * 3, ang, self.HUD_3D2DScale)
		cam.IgnoreZ(true)
			local FT = FrameTime()
			
			if self.dt.State == CW_AIMING or (self.InactiveWeaponStates[self.dt.State] and not (self.IsReloading and self.Cycle <= 0.98)) then
				self.HUD_3D2DAlpha = math.Approach(self.HUD_3D2DAlpha, 0, FT * 2000)
			else
				self.HUD_3D2DAlpha = math.Approach(self.HUD_3D2DAlpha, 255, FT * 2000)
			end
			...
			if mag <= self.Primary.ClipSize * 0.15 or reloadProgress then
...
function SWEP:getDifferenceToAimPos(pos, ang, vertDependance, horDependance, dependMod)
	...
	local sway = (self.AngleDelta.p * 0.2 * vertDependance + self.AngleDelta.y * 0.2 * horDependance) * 0.05 * dependMod
...
function SWEP:processSwayDelta(deltaTime, eyeAngles)
	if self:isBipodIdle() then
		...
		if self.SwayInterpolation == "linear" then
			self.AngleDelta = LerpAngle(math.Clamp(FT * 9, 0, 1), self.AngleDelta, delta)
...
function SWEP:performViewmodelMovement()
	...
	if self.dt.State == CW_AIMING then
		...
		self.ApproachSpeed = math.Approach(self.ApproachSpeed, (self.ADSSpeed * 20), FT * 500)
	...
	elseif self.dt.State == CW_RUNNING or (((len > ws * self.RunStateVelocity and self.Owner:KeyDown(IN_SPEED)) or len > ws * 3 or (self.ForceRunStateVelocity and len > self.ForceRunStateVelocity)) and self.Owner:OnGround()) then
		...
		if ((self.IsReloading or self.IsFiddlingWithSuppressor) and self.Cycle < 0.9) or self.reloadingM203 then
			...
			if self.SprintingEnabled then
				...
				if GetConVarNumber("cw_alternative_vm_pos") > 0 then
			...
			self.ApproachSpeed = math.Approach(self.ApproachSpeed, (self.ADSSpeed * 7), FT * 200)
		end
		...
		self.RunTime = self.RunTime + FT * (8 + math.Clamp(len / 120, 0, 5))
		local runTime = self.RunTime
		sin1 = math.sin(runTime) * mul
		sin2 = math.sin(runTime * 2 - 10) * mul
		cos1 = math.cos(runTime) * mul
		tan1 = math.atan(cos1 * sin1, cos1 * sin1) * mul
		pow1 = math.atan(2 * -sin1 ^ 4, alt - 90.0) * mul
		
		if self.MoveType == 1 then -- pistol VM movement
			AngMod.x = AngMod.x + tan1 * 8 * self.ViewModelMovementScale * mul
			AngMod.y = AngMod.y - cos1 * -6 * self.ViewModelMovementScale * mul
			AngMod.z = AngMod.z + sin1 * -10 * self.ViewModelMovementScale * mul
			
			PosMod.x = PosMod.x - sin1 * -0.8 * self.ViewModelMovementScale * mul
			PosMod.y = PosMod.y + pow1 * 1.8 * self.ViewModelMovementScale * mul
			PosMod.z = PosMod.z + tan1 * -1.5 * self.ViewModelMovementScale * mul
		else
			-- AngMod.x = AngMod.x + cos1 * -3 * self.ViewModelMovementScale * mul
			-- AngMod.y = AngMod.y - cos1 * -1 * self.ViewModelMovementScale * mul
			-- AngMod.z = AngMod.z + tan1 * 20 * self.ViewModelMovementScale * mul

			-- PosMod.x = PosMod.x - cos * -1 * self.ViewModelMovementScale * mul
			-- PosMod.y = PosMod.y + cos * -1 * self.ViewModelMovementScale * mul
			-- PosMod.z = PosMod.z + tan1 * -2 * self.ViewModelMovementScale * mul
			
			AngMod.x = AngMod.x + sin2 * -2 * self.ViewModelMovementScale * mul
			AngMod.y = AngMod.y - cos1 * 1.5 * self.ViewModelMovementScale * mul
			AngMod.z = AngMod.z + tan1 * 20 * self.ViewModelMovementScale * mul

			PosMod.x = PosMod.x - cos1 * -0.7 * self.ViewModelMovementScale * mul
			PosMod.y = PosMod.y + cos1 * -0.7 * self.ViewModelMovementScale * mul
			PosMod.z = PosMod.z + self:scaleMovement(-sin1 ^ 4, alt + 0.0001) * 0.1
		end
	...
	-- SWEP.ProneAng = Vector(5.618, -49.056, -15.311)
		...
		if self.dt.State == CW_CUSTOMIZE then
			...
			if self.NearWallEnabled then
				...
				td.endpos = td.start + self.Owner:EyeAngles():Forward() * 35
				...
				if tr.Hit or (IsValid(tr.Entity) and not tr.Entity:IsPlayer()) then
					...
					TargetPos = self.SprintPos * (1.3 - tr.Fraction)
					TargetAng = self.SprintAng * (0.8 - tr.Fraction)
					self.NearWall = true
				end
			end
		end
		
		self.ApproachSpeed = math.Approach(self.ApproachSpeed,  (self.ADSSpeed * 3) + 5, FT * 100)
	...
	elseif len > 10 and len < ws * 1.2 then
		...
		AngMod.z = AngMod.z + cos1 -5 + self:scaleMovement(sin1 * -10, mod2)
		AngMod.x = AngMod.x + tan - 0 + self:scaleMovement(tan1 * 2, mod2)
		...
		PosMod.z = PosMod.z + 0.5
				...
				if len < ws * 0.7 then
					AngMod.x = AngMod.x + self:scaleMovement(-sin1 ^ 8, alt + 2.0) -- up/down
				else
					AngMod.x = AngMod.x + self:scaleMovement(tan1 * 2, -alt + 3.0) / 1.5 -- up/down
				end
		AngMod.y = AngMod.y + self:scaleMovement(sin1 * 0.4, tan) -- left/right
		AngMod.z = AngMod.z + self:scaleMovement(tan1 * 3, mod2) -- rotation left/right
		...
		PosMod.z = PosMod.z - self:scaleMovement(tan1 * 0.2, mod2) -- up/down
	...
	self.BlendPos = LerpVectorCW20(FT * self.ApproachSpeed * 0.6, self.BlendPos, TargetPos)
	self.BlendAng = LerpVectorCW20(FT * self.ApproachSpeed * 1.2, self.BlendAng, TargetAng)
	
	-- the viewmodel movement position of the weapon
	if self.dt.State == CW_AIMING then
	CurPosMod = LerpVectorCW20(0.4, CurPosMod, PosMod)
	CurAngMod = LerpVectorCW20(0.1, CurAngMod, AngMod)
	else
	...
	if self.LuaViewmodelRecoil then
		...
		self.RecoilPos2 = LerpVectorCW20(FT * self.RecoilRestoreSpeed * 0.7, self.RecoilPos2, self.RecoilPos)
		self.RecoilAng2 = LerpAngleCW20(FT * self.RecoilRestoreSpeed * 0.8, self.RecoilAng2, self.RecoilAng)
...
function SWEP:makeVMRecoil(mod)
	...
	local overallMul = 0.25 + 0.9 * self.LuaVMRecoilIntensity * self.LuaVMRecoilMod
	
	-- get the offset multipliers
	local vertMul = math.Rand(0.13, 0.4) * overallMul * 2 * mod
...
function SWEP:GetViewModelPosition(pos, ang)
	pos = pos + ang:Forward() * -10000 -- move the default viewmodel away
	pos = pos + ang:Up() * 1000 -- move the default viewmodel away
	return pos, ang
end
