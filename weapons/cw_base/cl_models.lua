	...
	if att then
		if self.ShellDelay then
			CustomizableWeaponry.actionSequence.new(self, self.ShellDelay, nil, function()
				...
				-- if self.Links == true then
				-- CustomizableWeaponry.links.make(self, att.Pos + dir * self.ShellOffsetMul, EyeAngles(), dir * 100, 0.3, 8)
				-- end
		...
		if muz then
			...
			if muz2 then
				...
				if self.dt.Suppressed then
					...
					dlight.Pos = pos + self.Owner:GetAimVector() * 10
					dlight.Size = 300
					dlight.Decay = 50
...
function SWEP:processFOVChanges(deltaTime)
	if self.dt.State == CW_AIMING then
		self.CurVMFOV = Lerp(deltaTime * 10, self.CurVMFOV, self.AimViewModelFOV)
	else
		self.CurVMFOV = Lerp(deltaTime * 10, self.CurVMFOV, self.ViewModelFOV_Orig)
	end
...
function SWEP:performViewmodelMovement()
	...
	elseif self.dt.State == CW_RUNNING or (((len > ws * self.RunStateVelocity and self.Owner:KeyDown(IN_SPEED)) or len > ws * 3 or (self.ForceRunStateVelocity and len > self.ForceRunStateVelocity)) and self.Owner:OnGround()) then
		...
		if ((self.IsReloading or self.IsFiddlingWithSuppressor) and self.Cycle < 0.9) or self.reloadingM203 then
			...
			self.ApproachSpeed = math.Approach(self.ApproachSpeed, 4, FT * 200)
		end
		...
		self.RunTime = self.RunTime + FT * (7 + math.Clamp(len / 100, 0, 5))
		...
		if self.MoveType == 1 then -- pistol VM movement
			...
			AngMod.z = AngMod.z + cos1 * -10 * self.ViewModelMovementScale * mul
			PosMod.x = PosMod.x - sin1 * -0.8 * self.ViewModelMovementScale * mul
			PosMod.y = PosMod.y + tan1 * 1.8 * self.ViewModelMovementScale * mul
			PosMod.z = PosMod.z + tan1 * 0.5 * self.ViewModelMovementScale * mul
		else
			AngMod.x = AngMod.x + cos1 * -3 * self.ViewModelMovementScale * mul
			AngMod.y = AngMod.y - tan1 * -1.2 * self.ViewModelMovementScale * mul
			AngMod.z = AngMod.z + tan1 * 9 * self.ViewModelMovementScale * mul

			PosMod.x = PosMod.x - cos1 * -1 * self.ViewModelMovementScale * mul
			PosMod.y = PosMod.y + cos1 * -1 * self.ViewModelMovementScale * mul
		...
		if self.dt.State == CW_CUSTOMIZE then
			...
			td.endpos = td.start + self.Owner:EyeAngles():Forward() * 120
			...
			if tr.Hit or (IsValid(tr.Entity) and not tr.Entity:IsPlayer()) then
				...
				TargetPos = self.SprintPos * (0.5 + (tr.Fraction * -0.2))
				self.NearWall = true
			end
		end
		
		self.ApproachSpeed = math.Approach(self.ApproachSpeed, 8, FT * 10)
	...
	elseif len > 10 and len < ws * 1.2 then
		...
		AngMod.y = AngMod.y + self:scaleMovement(cos1 * -3.5, mod2) -- left/right
		AngMod.z = AngMod.z + self:scaleMovement(tan1 * 3, mod2) -- rotation left/right
		PosMod.x = PosMod.x + self:scaleMovement(sin1 * 0.18, mod2) -- left/right
		...
		local norm = math.Clamp(vel:GetNormal():DotProduct(self.Owner:EyeAngles():Forward()), 0.5, 1)
