...
SWEP.M203ReloadSounds = {{time = 0.0, sound = "CWC_FOLEY_MEDIUM"},
	{time = 0.3, sound = "CWC_M203_OPEN"},
	{time = 0.4, sound = "CWC_M203_REMOVE"},
	{time = 0.8, sound = "CWC_M203_SHELL"},
	{time = 1.0, sound = "CWC_FOLEY_TOSS"},
	{time = 1.3, sound = "CWC_M203_POSITION"},
	{time = 1.4, sound = "CWC_M203_INSERT"},
	{time = 2.05, sound = "CWC_M203_CLOSE"},
	{time = 2.2, sound = "CWC_FOLEY_LIGHT"}}
...
if CLIENT then
	...
	SWEP.SprintAng = Vector(-16.778, 20.573, 0)
	...
	SWEP.FireModeDisplayPos = "right"
	SWEP.SelectFont = "CW_SelectIcons"
	SWEP.SwimPos = Vector(1, -20, -20)	
	SWEP.SwimAng = Vector(90, -90.0, -70)	
	...
	SWEP.BreathHoldVelocityMinimum = 250 -- if our velocity surpasses this, we can't hold our breath
	...
	SWEP.NXSAxisAlign = {right = 0, up = 0, forward = 0}
end

-- SWEP.FireSound = "CWC_AR_15_FIRE"
-- SWEP.FireSoundSuppressed = "CWC_AR_15_FIRE_SUPPRESSED"

SWEP.WooPos = Vector(0.0, 0.0, 0.0)
SWEP.WooAng = Vector(0, 0, 0)
	
SWEP.FadeCrosshairOnAim = true
SWEP.CUSTOMIZATION_MENU_TOGGLE_WAIT = 0.2 -- the time to delay every action by when opening/closing the customization menu
SWEP.ADSSpeed = 1
SWEP.ADSSpeed_Orig = 1
...
SWEP.BASECaseLength = 50
SWEP.BASEBulletDiameter = 30
...
SWEP.ForcedHipWaitTime = 0.0 -- this is how much time the player will have to wait before being able to aim again after shooting with SWEP.ForceBackToHipAfterAimedShot set to true
...
function SWEP:CalculateEffectiveRange()
	self.EffectiveRange = self.BASECaseLength * 10 - self.BASEBulletDiameter * 5 -- setup realistic base effective range
...
function SWEP:getEffectiveRange()
	local EffectiveRange = self.BASECaseLength * 10 - self.BASEBulletDiameter * 5 -- setup realistic base effective range
	EffectiveRange = EffectiveRange * 39.37 -- convert meters to units

	local DamageFallOff = (100 - (self.BASECaseLength - self.BASEBulletDiameter)) / 200
	local PenStr = (self.BASEBulletDiameter * 0.5 + self.BASECaseLength * 0.35) * (self.PenAdd and self.PenAdd or 1)
...
function SWEP:Initialize()
	...
	if self.MuzzleVelocity then -- ENTER ONLY IN METER/S
		self.MuzzleVelocityConverted = self.MuzzleVelocity * 10
	end
...
function SWEP:unloadWeapon()
	...
	self:EmitSound "CWC_EMPTY"
	self:EmitSound "CWC_EMPTY2"
...
function SWEP:Holster(wep)
	...
	if self:filterPrediction() then
		if self.holsterSound then -- quick'n'dirty prediction fix
			self:EmitSound("CWC_HOLSTER", 70, 100)
...
function SWEP:CalculateSpread(vel, dt)
	...
	if not self.Owner.LastView then
		...
		self.Owner.ViewAff = LerpCW20(dt * 10, self.Owner.ViewAff, (aim - self.Owner.LastView):Length() * 1.5)
		self.Owner.LastView = aim
	end
	...
	if CT > self.SpreadWait then
		...
		self.AddSpreadSpeed = math.Clamp(self.AddSpreadSpeed + 50 * dt, 0, 1)
	end
end
		...
		if (SP and SERVER) or not SP then
			if self.dt.State == CW_AIMING then
				if not self.Owner:OnGround() or Length(GetVelocity(self.Owner)) >= self.Owner:GetWalkSpeed() * self.LoseAimVelocity or not self.Owner:KeyDown(IN_ATTACK2) then
					self.dt.State = CW_IDLE
					self:SetNextSecondaryFire(CT + 0.14)
					self:EmitSound("CWC_ADS")
				end
			end
		end
	end
						...
						if self.dt.State != CW_AIMING and self.dt.State != CW_CUSTOMIZE then
							if CT > self.FromActionToNormalWait then
								if self.dt.State != CW_IDLE then
									self:EmitSound("CWC_BKP")
									self.dt.State = CW_IDLE
									
									if not self.ReloadDelay then
										self:setGlobalDelay( 0.2 / self.ADSSpeed)
									end
								end
							end
						end
...
function SWEP:PrimaryAttack()
	...
	mag = self:Clip1()
		...
		if self.ActiveAttachments.am_luckylast and mag == 5 then
		self:EmitSound("CWC_LUCKYCOUNT1", 100, 100)
	end
		if self.ActiveAttachments.am_luckylast and mag == 4 then
		self:EmitSound("CWC_LUCKYCOUNT2", 100, 100)
	end
		if self.ActiveAttachments.am_luckylast and mag == 3 then
		self:EmitSound("CWC_LUCKYCOUNT3", 100, 100)
	end
		if self.ActiveAttachments.am_luckylast and mag == 2 then
		self:EmitSound("CWC_LUCKYCOUNT4", 100, 100)
	end
	
					-- A poor method of preventing sound/anim spasms when held down on burst fire modes.
						-- if self.BurstAmount > 1 then
						-- self:SetNextPrimaryFire(CT + 500)
						-- end
	
	if mag == 0  or (self.Owner:GetAmmoCount(self.Primary.Ammo) <= 0 and self.Primary.ClipSize == -1) then	
	
	 if self.FireMode == "semi" then
		self.Owner:ViewPunch(Angle(math.Rand(0.2, 0.6), math.Rand(-0.2, 0.2), math.Rand(-0.1, 0.1)))
		self:EmitSound("CWC_EMPTY", 100, 100)
		self:SetNextPrimaryFire(CT + 0.1)
	else
		self.Owner:ViewPunch(Angle(math.Rand(0.2, 0.6), math.Rand(-0.2, 0.2), math.Rand(-0.1, 0.1)))
		self:EmitSound("CWC_EMPTY", 100, 100)
		self:EmitSound("CWC_EMPTY2", 100, 100)
		self:SetNextPrimaryFire(CT + 0.3)
		end
	...
	if IsFirstTimePredicted() then
		...
		--Kinda need this one here for it to still work for the gimmick.
	if self.ActiveAttachments.cwc_horn and mag > 0 then
		self:EmitSound("CWC_SUBDOOT", 600, 600, 600)
end
	
	if self.ActiveAttachments.am_luckylast and mag == 1 then
		self:EmitSound("CWC_LUCKYLAST", 600, 600, 600)
		self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, 5)
	elseif self.ActiveAttachments.cwc_att_ammo_blossom then
		self:makeFireEffects()	
		self:EmitSound("CWC_BLOSSOMSUB")
		self:EmitSound("CWC_ECHO_MAG")
		self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, self.Shots)
		self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, self.Shots)
		self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, self.Shots)
		self:FireBullet(9, self.CurCone, 0.07, 4)
	else
			if mag == 1 then
					if GetConVarNumber("cwc_subsound_empty") > 0 then
						if self.CWCEmptySound == true then
							if self.Primary.ClipSize > 10 and self.Primary.ClipSize_Orig < 80 then
								self:EmitSound("CWC_LOWCOUNT", 100, 100)
								self:EmitSound("CWC_EMPTYFIRST", 200, 200)
							end
						end
					end	
			self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, self.Shots)
		...
		if GetConVarNumber("cwc_subsound_lowcount") > 0 then
				if mag < (self.Primary.ClipSize * 0.30) and (mag > 0) and self.Primary.ClipSize > 10 then
					self:EmitSound("CWC_LOWCOUNTSUB", 100, 100)
				end
			end	
	self:FireBullet(self.Damage, self.CurCone, self.ClumpSpread, self.Shots)
	end
		...
		self:makeFireEffects()	
		if GetConVarNumber("cw_freeaim") < 0  then -- This section is so the recoil doesnt derp downwards so much from the viewmodel when firing.
			if self.dt.State == CW_AIMING and self.Owner:KeyDown(IN_SPEED) then
				self.Owner:ViewPunch(Angle((self.Recoil*0.2), (self.Damage*math.Rand(-0.01, 0.01)), (self.Damage*math.Rand(-0.005, 0.005))))
			else
				self.Owner:ViewPunch(Angle((self.Recoil*1), (self.Damage*math.Rand(-0.01, 0.01)), (self.Damage*math.Rand(-0.02, 0.02))))	
			end
		else
			if self.dt.State == CW_AIMING and self.Owner:KeyDown(IN_SPEED) then
				self.Owner:ViewPunch(Angle(0, (self.Damage*math.Rand(-0.01, 0.01)), (self.Damage*math.Rand(-0.005, 0.005))))
			else
				self.Owner:ViewPunch(Angle((-self.Recoil*math.Rand(-0.5, 0.5)), (self.Damage*math.Rand(-0.01, 0.01)), (self.Damage*math.Rand(-0.02, 0.02))))	
			end
		end
		----------			
		if GetConVarNumber("cwc_subsound_pov") > 0 then
	if self.ActiveAttachments.am_magnum and not self.dt.Suppressed and mag > 0 then
		if self.Damage > 29 and self.SpeedDec > 29 then
		self:EmitSound("CWC_MAGNUMSUB", 600, 600, 600)
		else
		self:EmitSound("CWC_MAGNUMSUBLESSER", 600, 600, 600)
		end
end
	if self.ActiveAttachments.am_magnum and self.dt.Suppressed and mag > 0 then
		self:EmitSound("CWC_MAGNUMSUBSUPPRESSED", 600, 600, 600)

end
	if self.ActiveAttachments.am_matchgrade and mag > 0 then
		self:EmitSound("CWC_MATCHSUB", 600, 600, 600)

end
	if self.ActiveAttachments.am_flechetterounds then
		self:EmitSound("CWC_FLECHETTESUB", 600, 600, 600)

end
	if self.dt.Suppressed and mag > 0 then
		self:EmitSound("CWC_SUPSUB", 600, 600, 600)

end
end
	
			if mag > 0 and not self.dt.Suppressed and not self.ActiveAttachments.cwc_horn then
				if self.ActiveAttachments.am_magnum then
					if self.Damage > 29 and self.SpeedDec > 29 then
						self:EmitSound("CWC_DIST_MAG", 600, 600, 600)
					else
						self:EmitSound("CWC_DIST_LITE_MAG", 600, 600, 600)
					end
					self:EmitSound("CWC_ECHO_MAG", 600, 600, 600)
				else
					if self.Damage > 29 and self.SpeedDec > 29 then
						self:EmitSound("CWC_DIST", 600, 600, 600)
					else
						self:EmitSound("CWC_DIST_LITE", 600, 600, 600)
					end
					self:EmitSound("CWC_ECHO", 600, 600, 600)
				end
	end
end
----------
...
function SWEP:GetRecoilModifier(mod)
	...
	if not self.Owner:OnGround() then
		if self.FireDelay < 0.125 and self.FireMode == "auto" then
		mod = mod * -2
		else
		mod = mod * 2
		end
	...
	if self.FireMode == "semi" and self.Primary.ClipSize > 20 and self.SpeedDec > 20 then
		mod = mod * (self.BurstRecoilMul / 3)
	end
...
function SWEP:SecondaryAttack()
	...
	if self.dt.State ~= CW_AIMING then
		self.Owner:ViewPunch(Angle(math.Rand(-0.5, 1), 0, math.Rand(-1, -0.5)))
		self.dt.State = CW_AIMING
		
		if self:filterPrediction() then
			if self.ADSSpeed > 1 then
				self:EmitSound("CWC_FOLEY_TOSS")
				else
				self:EmitSound("CWC_ADS")
			end
		end
...
-- if self.ActiveAttachments.cwc_flashhider then
